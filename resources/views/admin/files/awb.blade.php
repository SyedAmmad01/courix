...
<head>
    ...
    <title>getairwaybilllabel</title>
...
<div class="container mt-5">
    <div class="row">
        <div class="col-12">
            <div class="outside">
                <h3 class="reference">
                    {{$lastShipment->reference_number}}
                </h3>
            </div>
        	{{-- <h3 class="outside">{{$lastShipment->reference_number}}</h3> --}}
        	<h3 class="outside_2">*{{$lastShipment->barcode}}*</h3>
            ...
            <div class="card border-dark">
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-4">
                            <div>{!! DNS1D::getBarcodeHTML("$lastShipment->barcode", 'C39', 2, 80) !!}</div>
                            <h2 style="margin-left: 80px;">{{$lastShipment->barcode}}</h2>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @php
                if($lastShipment->city == 1){
                    $last = 'AUH';
                }
                if($lastShipment->city == 2){
                    $last = 'SHJ';
                }
                if($lastShipment->city == 3){
                    $last = 'DXB';
                }
                if($lastShipment->city == 4){
                    $last = 'AJM';
                }
                if($lastShipment->city == 5){
                    $last = 'ALN';
                }
                if($lastShipment->city == 6){
                    $last = 'FUJ';
                }
                if($lastShipment->city == 7){
                    $last = 'UMQ';
                }
            @endphp
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-6 text-right">
                            <p>{{$lastShipment->reference_number}}</p>
                        </div>
                    </div>
                </div>
            </div>
			        ...
			        <div class="row no-gutters">
			            ...
			            <div class="col-3">
			                <div class="box1">
                                ...
                                @php
                                $cityCode = null;
                                if ($lastShipment->city !== null) {
                                    $city = DB::table('citys')->where('id', $lastShipment->city)->first();
                                    if ($city) {
			            ...
			            <div class="col-3">
			                <div class="box1">
			                	...
			                	<h5 class="text-center">{{$lastShipment->service_type}}</h5>
			                </div>
			            </div>
			            ...
			            <div class="col-4">
			                ...
			                <h5 class="text-center">{{$lastShipment->cod}}.00/-</h5>
			                </div>
			            </div>
			            ...
			            <div class="col-4">
			                ...
			                <h5 class="text-center">{{$lastShipment->no_of_peices}}</h5>
			                </div>
			            </div>
			...
			<div class="card-group">
			        ...
			        <div class="row no-gutters">
			            <div class="col-4">
			                <div class="box1">
                                @php
                                $shippers = null;
                                if ($lastShipment->shipper_code !== null) {
                                $shippers = DB::table('shippers')->where('id', $lastShipment->shipper_code)->first();
                                }
                                @endphp
			            ...
			            <div class="col-4">
			                <div class="box1">
			                	<p class="text-center">{{$lastShipment->order_date}}</p>
			                </div>
			            </div>
			    ...
			    <div class="card2">
			        <div class="box3">
                        <div class="rotated-text1 mb-3">{!! DNS1D::getBarcodeHTML("$lastShipment->reference_number", 'C93' ,1  ,50 ) !!}</div>
			        </div>
			    </div>
			</div>
			            ...
			            <div class="col-12">
			                ...
			                <div class="box6">
			                	<div class="row">
			                		...
			                		<div class="col-9">
                                        <h5>{{$lastShipment->reciver_name}}</h5>
                                        @php
                                        $country_reciver = DB::table('countrys')->where('id', $lastShipment->country)->first();
                                        $city_reciver = DB::table('citys')->where('id', $lastShipment->city)->first();
                                        $area_reciver = DB::table('areas')->where('id', $lastShipment->area)->first();
                                        @endphp
                                        <h5>{{ $shippers ? $lastShipment->street_address : 'Unknown Address' }} , {{ $area_reciver ? $area_reciver->name : 'Unknown Area' }} , {{ $city_reciver ? $city_reciver->name : 'Unknown City' }} , {{ $country_reciver ? $country_reciver->name : 'Unknown Country' }}</h5>
			                			 <h5>{{ $area_reciver ? $area_reciver->name : 'Unknown Area' }}</h5>
			                			 <h5>{{ $city_reciver ? $city_reciver->name : 'Unknown City' }}</h5>
			                			 <h5>{{$lastShipment->mobile_1}}</h5>
			                			 <h5>{{$lastShipment->mobile_2}}</h5>
			                		</div>
			                	</div>
			                </div>
			            </div>
			            ...
			            <div class="col-12">
			                <div class="box1">
			                	<div class="row">
			                		...
			                		<div class="col-6">
			                			 <h5>{{$lastShipment->instruction}}</h5>
			                			 <h5>{{$lastShipment->account_name}}</h5>
			                			 <h5>{{$lastShipment->reference_number}}</h5>
			                			 <h5>{{ $current_time }}</h5>
			                		</div>
